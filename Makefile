...
#? #####################每次执行make指令这一部分都会执行##########################?
#? STEP 01: uboot版本号  赋值
VERSION = 2009 	#!主版本号，2009
PATCHLEVEL = 11 #!补丁级别版本号，11
SUBLEVEL = 		#!次版本号（此处为空）
EXTRAVERSION = 	#!附加版本号（此处为空）
ifneq "$(SUBLEVEL)" "" #!判断 SUBLEVEL 是否为空
U_BOOT_VERSION = $(VERSION).$(PATCHLEVEL).$(SUBLEVEL)$(EXTRAVERSION)
else
U_BOOT_VERSION = $(VERSION).$(PATCHLEVEL)$(EXTRAVERSION)#! U_BOOT_VERSION 的值为 2009.11。
endif

#? STEP 02: TIMESTAMP_FILE 和 VERSION_FILE时间生成脚本和版本生成脚本  先赋值
TIMESTAMP_FILE = $(obj)include/timestamp_autogenerated.h
VERSION_FILE = $(obj)include/version_autogenerated.h


#? STEP 03: 主机系统架构  HOSTARCH = arm 去查看下s3c2440的架构
#! $(shell ...)：这是 Makefile 中的一个函数，用于执行 shell 命令并将输出作为 Makefile 中的值。它会执行 uname -m 命令，并将结果传递给 sed 命令进行进一步处理。
#! sed：一个流编辑器，用于对输入数据进行文本替换。  aarch64
#! -e s/arm.*/arm/：将 arm 相关的所有输出标准化为 arm。例如，将 armv7l、arm64 等替换为 arm。
HOSTARCH := $(shell uname -m | \
	sed -e s/i.86/i386/ \
	    ...
	    -e s/ppc64/ppc/ \
	    -e s/macppc/ppc/)
#? STEP 04: 获取主机操作系统  HOSTOS = linux
#! tr '[:upper:]' '[:lower:]'：将所有大写字母转换为小写字母。这样做可以确保操作系统名称以小写字母表示，使后续处理更统一。
#! Linux ---> linux
#! 会在文本中查找包含 "cygwin" 的内容，并将其替换为 cygwin，删除 cygwin 后面的所有内容。
HOSTOS := $(shell uname -s | tr '[:upper:]' '[:lower:]' | \
	    sed -e 's/\(cygwin\).*/cygwin/')

# Set shell to bash if possible, otherwise fall back to sh
#? STEP 05: 选择一个可用的 Shell 程序
#! if [ -x "$$BASH" ]：检查环境变量 BASH 指向的文件是否存在且可执行。
#! fi：用于结束 if 语句的语法。
SHELL := $(shell if [ -x "$$BASH" ]; then echo $$BASH; \
	else if [ -x /bin/bash ]; then echo /bin/bash; \
	else echo sh; fi; fi)
#! export 命令用于将变量导出到环境中，使得这些变量在 Makefile 中调用的子进程或 shell 脚本中也能使用。
#! HOSTARCH:arm
#! HOSTOS:linux
#! SHELL:sh
...
#? STEP 06: 根据终端命令是否是make -s来决定是否输出命令本身的执行过程
#! MAKEFLAGS 是 make 的一个内部变量，用于存储传递给 make 的命令行选项
#! 运行 make -s 时，-s 选项会被添加到 MAKEFLAGS 中，因此 MAKEFLAGS 的值中包含了字符 s。这会使得 make 在执行命令时不输出命令本身的执行过程，只显示最终的结果或错误信息。
...
#
#? STEP 07: 如果 O 已定义且来自命令行，则将 BUILD_DIR 设置为 O 的值
#! 如果 O 已定义且来自命令行，则将 BUILD_DIR 设置为 O 的值
ifdef O
...
#? STEP 08: 检测之前指定的BUILD_DIR路径是否存在，如果不存在则创建
#! -d 是一个用于检查目录是否存在的条件
#! || 是 shell 中的逻辑 OR 运算符。它的含义是：如果前面的命令 [ -d ${BUILD_DIR} ] 返回 false（即目录不存在），那么执行后面的命令 mkdir -p ${BUILD_DIR}。
$(shell [ -d ${BUILD_DIR} ] || mkdir -p ${BUILD_DIR})

# Verify if it was successful.
#? STEP 09: 将BUILD_DIR变量的值设置为该目录的绝对路径，并且检测BUILD_DIR是否为空，如果是空则报错
#! 将 BUILD_DIR 变量的值设置为该目录的绝对路径
BUILD_DIR := $(shell cd $(BUILD_DIR) && /bin/pwd)
$(if $(BUILD_DIR),,$(error output directory "$(saved-output)" does not exist))
endif # ifneq ($(BUILD_DIR),)

#? STEP 10: 如果没有指定输出路径，则TOPDIR SRCTREE OBJTREE都是同一路径，并输出TOPDIR SRCTREE OBJTREE
#! $(CURDIR) 是 Makefile 的内置变量，表示当前执行 make 的工作目录的绝对路径。 构建输出的目录
...
#? STEP 11: 输出MKCONFIG
#! 获取当前执行make的工作目录下的mkconfig
MKCONFIG	:= $(SRCTREE)/mkconfig
export MKCONFIG
#? STEP 12: 根据OBJTREE和SRCTREE的路径，来决定REMOTE_BUILD的值，并输出REMOTE_BUILD
#! 如果 OBJTREE 与 SRCTREE 不同：表示进行了独立构建。
#! 如果 OBJTREE 与 SRCTREE 相同：表示在源代码目录内进行构建。
...
#? STEP 13: 根据OBJTREE和SRCTREE的路径，来决定obj和src的值，相等都为空，并输出obj src
ifneq ($(OBJTREE),$(SRCTREE))
obj := $(OBJTREE)/
src := $(SRCTREE)/
else
...
#? STEP 14: 禁止输出CDPATH
#! 用于防止 CDPATH 变量被传递到子进程的命令
unexport CDPATH

#########################################################################
#? STEP 15: 根据ARCH的值与powerpc比较，如果相同则赋值ARCH为ppc，这里ARCH应该还是空
ifeq ($(ARCH),powerpc)
ARCH = ppc
endif
...
ifeq ($(HOSTARCH),$(ARCH))
CROSS_COMPILE ?= #! ?= 是 Makefile 中的条件赋值运算符，它表示只有当 CROSS_COMPILE 没有被赋值时，才会将其设置为空。
endif

# load other configuration 
#! config.mk 中的内容会被立即执行，而不是在后续某个时刻才执行。
...
#!为$(OBJS) 列表中的每个目标文件添加 $(obj) 作为前缀，使得文件路径变成相对路径或者完整路径。
OBJS := $(addprefix $(obj),$(OBJS))

LIBS  = lib_generic/libgeneric.a
LIBS += lib_generic/lzma/liblzma.a
LIBS += lib_generic/lzo/liblzo.a
#!检测board/samsung/common/Makefile是否存在，如果存在就打印
...
#!TIMESTAMP_FILE = $(obj)include/timestamp_autogenerated.h
#!VERSION_FILE = $(obj)include/version_autogenerated.h
#!.PHONY 表示后面列出的目标是伪目标，不是文件。伪目标不会因为有同名文件存在而受到影响，它们总是会被执行。
...
#!用于指定编译时链接的 libgcc 库位置
#!-L：告诉编译器在指定的目录中查找库文件。此处的 -L 后面跟的是 libgcc 库所在的目录路径
#!dirname 是一个命令，用于提取文件路径中的目录部分。
#!-lgcc：指定要链接的库是 libgcc。编译器会在上面通过 -L 指定的目录中查找 libgcc 文件
...
#!$@：这是 Makefile 中的一个特殊变量，它表示当前规则的目标。在这个例子中，它指的是 $(VERSION_FILE)，也就是我们要生成的文件
$(VERSION_FILE):
...
#!-x c：强制编译器将文件视为 C 语言文件，即使文件扩展名不是 .c 也可以解析。
#!-DDO_DEPS_ONLY：定义宏 DO_DEPS_ONLY，这个宏可能在头文件中控制一些条件编译逻辑，只在生成依赖项时有效。
#!HOSTCFLAGS	= -Wall -Wstrict-prototypes -O2 -fomit-frame-pointer
#!-M 参数告诉编译器生成依赖项，不进行实际的编译
#!-MQ $(obj)include/autoconf.mk：-MQ 用于生成目标文件的依赖规则，告诉编译器输出一个包含目标文件 autoconf.mk 的依赖行。-MQ 确保特殊字符（如空格、$符号等）能正确地出现在依赖文件中。
#!include/common.h：依赖的输入文件，编译器会检查这个文件的依赖项。
#!> $@：将生成的依赖关系输出到目标文件 $@，即 autoconf.mk.dep
#!目标文件：autoconf.mk.dep，负责存储头文件 config.h 和 common.h 的依赖关系
...
#!$(XECHO) 是一个打印命令，用来显示正在生成的文件。$@ 表示当前目标文件，即 $(obj)include/autoconf.mk
#!set -e 命令确保脚本在执行过程中如果遇到任何错误（非零的返回值），会立即退出，防止执行后续操作。
#!: Extract the config macros ;这一行是一个注释，说明接下来会提取配置宏。
#!CPP	= $(CC) -E  = gcc -E
#!DDO_DEPS_ONLY 是一个预处理宏，用于标识在这个步骤中只处理依赖关系。
#!-dM 是 CPP 的一个选项，用来显示所有的宏定义。它会预处理 include/common.h 文件并输出所有宏定义。这里没有进行实际编译，只是生成宏列表。
#!预处理器的输出会通过 sed 过滤处理。define2mk.sed 脚本会将宏定义转换成 Makefile 格式，然后重定向到 $@.tmp 文件，也就是临时文件 $(obj)include/autoconf.mk.tmp。
#!如果上一步成功，临时文件将会被移动为最终的 $(obj)include/autoconf.mk，这可以避免生成过程中出现问题导致半生成文件污染目标文件。
